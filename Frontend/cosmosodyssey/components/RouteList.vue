<template>
          ...
          >
            ...
            <TableCell>
              <UButton
                ...
                @click="openReservationModal(route.providerId)"
              ></UButton>                                                                                                                             
            </TableCell>
          </TableRow>
        </TableBody>
  ...
  <UModal v-model="isReservationModalOpen" prevent-close teleport="#modal">
    <UCard >
      <template #header>
        <div class="flex items-center justify-between">
          <UFormField label="Make a reservation" name="makeReservation"></UFormField>
          <UButton
            variant="ghost"
      ...
      <template #footer>
        <div class="flex justify-end space-x-2">
          ...
          <UButton @click="submitReservation">Submit</UButton>
        </div>
      </template>
    </UCard>
  </UModal>
...
onMounted(async () => {
  if (routes.value.length === 0) {
    ...
    const exists = await routeInfoStore.checkRouteExists(origin, destination);
...
.search-input {
  ...
  width: 300px; 
  padding: 8px 12px; 
  border: 1px solid rgb(202, 7, 111);
  border-radius: 4px; 
  box-shadow: 0 4px 4px rgb(202, 7, 111); 
  font-size: 15px; 
}
.search-input:focus {
  outline: none; 
  border-color:rgb(202, 7, 111); 
  box-shadow: 0 0px 5px rgba(202, 7, 111); 
  z-index: 1;
}
